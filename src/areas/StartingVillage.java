...
/**
 * The first village in a world, where a player arrives into the world.
 *
 * User: zadjii
 * Date: 10/30/13
 */
public class StartingVillage {
	...
	public static int[][] generateVillage(
		...
		int villageStartX = 0;
		int villageStartY = 0;
		//FIXME I don't know what I'm doing, but I'm guaranteeing that the village is hueg?
		size = 12;
		...
		if(villageActualSize > areaSize){
			System.err.println("SV:00: Village too big. Ignoring.");
//			return null;
		}
		if( (areaSize-villageActualSize) < areaSize/4 ){
			System.err.println("SV:01: Village too big. Ignoring.");
//			return null;
		}
...
//			return null;
		...
		try{overview[cx][cy-3]=ROAD;}catch (IndexOutOfBoundsException e){/*nothing*/}
		try{overview[cx][cy-4]=ROAD;}catch (IndexOutOfBoundsException e){/*nothing*/}
		try{overview[cx-1][cy-3]=ROAD;}catch (IndexOutOfBoundsException e){/*nothing*/}
		try{overview[cx-2][cy-5]=TOWNCENTER;}catch (IndexOutOfBoundsException e){/*nothing*/}
		try{overview[cx-2][cy-4]=TOWNCENTER;}catch (IndexOutOfBoundsException e){/*nothing*/}
		try{overview[cx-1][cy-5]=TOWNCENTER;}catch (IndexOutOfBoundsException e){/*nothing*/}
		try{overview[cx-1][cy-4]=TOWNCENTER;}catch (IndexOutOfBoundsException e){/*nothing*/}
