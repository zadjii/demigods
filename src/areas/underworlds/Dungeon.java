...
public class Dungeon extends NewGameArea {


	public static final int SIZE = 512;

	public Dungeon(Random rand, int size, int ID){
		super(rand, SIZE, ID);
		init();
	...
	/*
	 * public Dungeon(Random rand, int size, boolean newDungeon, int seed){
	 * this.rand = rand; this.size = size; this.SEED = seed;
	 * ++Dungeon.mapIDNext; this.ID = Dungeon.mapIDNext; if(newDungeon){
	 * this.map = generateNewCave(size); makeWalls(); } tileMap =
	 * this.toTileDungeon();
	 *
	 * } //
	 */
	...
	public int[][] generateNewCave(int size) {
		...
		while (iterations < numberOfIterations){
			...
			if(iterations > 500)innerRadiusRangeAddition += (int)((20)*((float)iterations/(float)numberOfIterations));
